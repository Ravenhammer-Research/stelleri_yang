...
public:
    explicit YangSchemaModule(const struct lys_module *m = nullptr) noexcept : mod_(m) {}

    const struct lys_module* raw() const noexcept { return mod_; }
    const char* name() const noexcept { return mod_ ? mod_->name : nullptr; }
    const char* revision() const noexcept { return mod_ ? mod_->revision : nullptr; }
    const char* ns() const noexcept { return mod_ ? mod_->ns : nullptr; }
    const char* prefix() const noexcept { return mod_ ? mod_->prefix : nullptr; }
    const char* filepath() const noexcept { return mod_ ? mod_->filepath : nullptr; }
    const char* org() const noexcept { return mod_ ? mod_->org : nullptr; }
    const char* contact() const noexcept { return mod_ ? mod_->contact : nullptr; }
