...
// Each enum member corresponds to a libyang LY_CTX_* macro (see libyang/context.h).
// Brief descriptions are included here for convenience.
enum class YangContextOptions : unsigned {
    ...
    // (implement modules referenced by when/must/defaults so referenced nodes work).
    RefImplemented = LY_CTX_REF_IMPLEMENTED,

    // LY_CTX_NO_YANGLIBRARY (0x04)
    // Do not internally implement ietf-yang-library module; ly_ctx_get_yanglib_data()
    // will not work until the module is loaded manually.
    ...
    // When searching for schema files prefer configured searchdirs over callback.
    ...
    // Include obsolete nodes in compiled schema so they can be created/validated.
    ...
    // Restrict loading to static (built-in) plugins and ignore external plugin dirs.
...
inline constexpr unsigned toFlags(YangContextOptions o) noexcept { return static_cast<unsigned>(o); }
