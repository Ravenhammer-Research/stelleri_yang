  ...
  public:
    ...
    struct Route {
      ...
      std::optional<NextHop>
          next_hop; // container next-hop (uses next-hop-state-content)
    ...
    struct Rib {
      ...
      bool default_rib =
          true; // if-feature multiple-ribs; config false in model
    ...
    static std::unique_ptr<IetfRouting> deserialize(const YangContext &ctx,
