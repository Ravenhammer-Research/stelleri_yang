...
<ietf-routing:routing xmlns:ietf-routing="urn:ietf:params:xml:ns:yang:ietf-routing">
    ...
    <ribs>
        ...
        if (out.control_plane_protocols[0].static_routes.size() == 0) {
                ...
                ATF_REQUIRE(out.control_plane_protocols[0].static_routes[0].route_preference.has_value());
                ATF_REQUIRE(*out.control_plane_protocols[0].static_routes[0].route_preference == 10);
        }
...
ATF_INIT_TEST_CASES(tcs) {
    ATF_ADD_TEST_CASE(tcs, ietf_routing_roundtrip);
}
