...
static void libyang_log_cb(LY_LOG_LEVEL level, const char *msg, const char *data_path,
...
ATF_TEST_CASE_BODY(yang_context_raw) {
    /* Set the callback and increase verbosity to debug to ensure all messages are emitted. */
    ...
    const std::vector<std::string> testPaths = { "/usr/local/share/yang/modules/yang/standard/ietf/RFC/" };
    ...
    (void)msg;
        ...
        for (const auto &p : testPaths) {
            if (std::find(paths.begin(), paths.end(), p) != paths.end()) { any_found = true; break; }
        }
        if (!any_found) ATF_FAIL("none of the test search paths were added to the context");
    ...
    if (!mod) ATF_FAIL("ietf-ip module not found in libyang context");
}
