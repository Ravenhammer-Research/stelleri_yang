...
struct lyd_node *IetfRouting::serialize(const YangContext &ctx) const {
  ...
  LY_ERR err =
      lyd_new_path(nullptr, c, "/ietf-routing:routing", NULL, 0, &root);
  ...
  if (routing_.router_id.has_value()) {
    {
      ...
      lyd_new_path(root, c, "router-id", routing_.router_id->c_str(), 0, &tmp);
    }
  }
  ...
  for (const auto &ifname : routing_.interfaces) {
    {
      struct lyd_node *tmp = nullptr;
      lyd_new_path(root, c, "interfaces/interface", ifname.c_str(), 0, &tmp);
    }
  }
  ...
  for (const auto &cpp : routing_.control_plane_protocols) {
    ...
    {
      struct lyd_node *tmp = nullptr;
      lyd_new_path(root, c, (pred + "/type").c_str(), cpp.type.c_str(), 0,
                   &tmp);
    }
    {
      struct lyd_node *tmp = nullptr;
      lyd_new_path(root, c, (pred + "/name").c_str(), cpp.name.c_str(), 0,
                   &tmp);
    }
    if (cpp.description.has_value()) {
      {
        struct lyd_node *tmp = nullptr;
        lyd_new_path(root, c, (pred + "/description").c_str(),
      ...
      if (r.route_preference.has_value()) {
        {
          ...
          lyd_new_path(root, c,
                       ...
                       std::to_string(*r.route_preference).c_str(), 0, &tmp);
        }
      }
    }
  }
  ...
  for (const auto &rib : routing_.ribs) {
    ...
    {
      struct lyd_node *tmp = nullptr;
      lyd_new_path(root, c, (pred + "/name").c_str(), rib.name.c_str(), 0,
                   &tmp);
    }
    {
      struct lyd_node *tmp = nullptr;
      lyd_new_path(root, c, (pred + "/address-family").c_str(),
      ...
      if (r.route_preference.has_value()) {
        {
          struct lyd_node *tmp = nullptr;
          lyd_new_path(root, c,
                       (pred + "/routes/route/route-preference").c_str(),
                       std::to_string(*r.route_preference).c_str(), 0, &tmp);
        }
      }
    }
...
std::unique_ptr<IetfRouting> IetfRouting::deserialize(const YangContext &ctx,
  ...
  if (c && (v = lyd_get_value(c)))
    ...
    if (lyd_find_path(tree, "/ietf-routing:routing/router-id", 0, &abs_c) ==
            LY_SUCCESS &&
        abs_c && (v = lyd_get_value(abs_c))) {
      model->mutableRouting().router_id = std::string(v);
    }
  }
  ...
  if (ifs) {
    for (struct lyd_node *ch = lyd_child(ifs); ch; ch = ch->next) {
      ...
      if (strcmp(ch->schema->name, "interface") == 0) {
        if ((v = lyd_get_value(ch)))
          model->mutableRouting().interfaces.push_back(std::string(v));
      }
    }
  }
  ...
  if (cpps) {
    for (struct lyd_node *entry = lyd_child(cpps); entry; entry = entry->next) {
      ...
      struct lyd_node *t = find_child(entry, "type");
      if (t && (v = lyd_get_value(t))) {
        std::string sv(v);
        auto pos = sv.find(':');
      ...
      struct lyd_node *nnode = find_child(entry, "name");
      if (nnode && (v = lyd_get_value(nnode)))
        cp.name = v;
      struct lyd_node *d = find_child(entry, "description");
      if (d && (v = lyd_get_value(d)))
      ...
      if (sr) {
        for (struct lyd_node *r = lyd_child(sr); r; r = r->next) {
          ...
          struct lyd_node *rp = find_child(r, "route-preference");
          if (rp && (v = lyd_get_value(rp))) {
  ...
  if (ribs) {
    for (struct lyd_node *entry = lyd_child(ribs); entry; entry = entry->next) {
      ...
      if (nnode && (v = lyd_get_value(nnode)))
        rib.name = v;
      struct lyd_node *af = find_child(entry, "address-family");
      if (af && (v = lyd_get_value(af)))
        rib.address_family = v;
      struct lyd_node *desc = find_child(entry, "description");
      if (desc && (v = lyd_get_value(desc)))
